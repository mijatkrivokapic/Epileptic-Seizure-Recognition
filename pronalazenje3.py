...
# %% [markdown]
# Podela na trening i test podatke i skaliranje podataka
...
print(f"Rezultati nad test podacima sa podrazumevanim parametrima: {f1_score(ft_y_test,y_pred)}")
...
ft_gs_results=[]
for i in range(5):
  current_result=find_best_svc_params(ft_components_split[i]['x_train'],ft_components_split[i]['y_train'],f"ft_{i+1}_gs_result.pkl",False)
  ft_gs_results.append(current_result)
